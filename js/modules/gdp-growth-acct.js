...
(function () {
  ...
  /* ── Own chart instance (update-in-place instead of destroy+recreate) ── */
  let _chart = null;
  ...
  function renderChart(tfp, cap, lab) {
    ...
    if (_chart) {
      // Update existing chart in-place — avoids Chart.js canvas reuse issues
      _chart.data.datasets[0].data = newData;
      _chart.update('none'); // 'none' = skip animation for instant response
      return;
    }

    // First render: destroy any chart mkChart may have created on this canvas
  ...
  /* ── Render labels + result text + chart ───────────────── */
  function render(periods) {
    ...
    const methodNote = d.live
      ? `<span style="font-size:.73rem;opacity:.55;margin-left:.4rem;">\u2713 SingStat \u00b7 PIM (\u03b4=${(delta*100).toFixed(1)}%) \u00b7 \u0394ln method \u00b7 ${d.n} obs</span>`
  ...
  window.initGDPGrowthAcct = async function () {
    ...
    try {
      ...
      if (window.growthData) Object.assign(window.growthData, _livePeriods);
      render(_livePeriods);
      setStatus('\u2713 SingStat \u00b7 PIM capital stock \u00b7 \u0394ln Solow residual \u00b7 M015721 \u00b7 M016161 \u00b7 M015761');
    } catch (err) {
      console.warn('[gdp-growth-acct] API error:', err.message);
