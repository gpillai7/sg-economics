...
(function () {
  ...
  const LOG = (...a) => console.log('[growth-acct]', ...a);
  ...
  function render(periods) {
    ...
    LOG('render called — alpha:', alpha, 'delta:', (delta*100).toFixed(1)+'%');
    ...
    const note=d.live
      ?`<span style="font-size:.73rem;opacity:.55;margin-left:.4rem;">\u2713 SingStat \u00b7 PIM (\u03b4=${(delta*100).toFixed(1)}%) \u00b7 ${d.n} obs</span>`
    ...
    if(rEl) rEl.innerHTML=
      ...
      `Capital (\u03b1\u00b7g_K): ${cap.toFixed(2)}pp &nbsp;|&nbsp; Labour: ${lab.toFixed(2)}pp &nbsp;|&nbsp; `+
  ...
  function attachListeners(){
    ...
    const aEl=document.getElementById('alpha'),pEl=document.getElementById('growth-period'),dEl=document.getElementById('delta-pim');
    ...
    LOG('listeners attached to alpha:', !!aEl, 'period:', !!pEl, 'delta:', !!dEl);
  }
  ...
  window.initGDPGrowthAcct = async function(){
    ...
    try{
      ...
      if(window.growthData) Object.assign(window.growthData,_livePeriods);
      render(_livePeriods);
      setStatus('\u2713 SingStat \u00b7 PIM \u00b7 M015721 \u00b7 M016161 \u00b7 M015761');
    }catch(err){
